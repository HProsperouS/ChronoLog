      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setSelectedCategory(cat)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedCategory === cat ? 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white' : 'bg-white/5 text-gray-400 border border-white/10 hover:bg-white/10'}`}
            >{cat}</button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-5">
          ...
          <div className="bg-[#13131a] border border-white/5 rounded-xl p-4">
            ...
            <p className="text-xl font-semibold text-white">{formatDuration(filteredActivities.reduce((s, a) => s + a.duration, 0))}</p>
          </div>
          <div className="bg-[#13131a] border border-white/5 rounded-xl p-4">
            <p className="text-xs text-gray-500 mb-1">Avg Session</p>
            <p className="text-xl font-semibold text-white">
              {filteredActivities.length > 0
                ? formatDuration(Math.round(filteredActivities.reduce((s, a) => s + a.duration, 0) / filteredActivities.length))
                : '0m'}
            </p>
          </div>
        </div>
                  ...
                  <div className="bg-white/5 border border-white/5 rounded-lg p-4 hover:bg-white/[0.07] hover:border-white/10 transition-all">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="text-right ml-4">
                        ...
                        <p className="text-[10px] text-gray-500 mt-0.5">{formatTime(activity.startTime)} – {formatTime(activity.endTime)}</p>
                      </div>
                    </div>
                    <div className="mt-3">
